---
title: "Instalación de R, Rstudio y SIVIREP"
...
## Si presenta inconvenientes o errores que no puedan ser resueltos con la guía, por favor comunicarse vía email a:
...
- Tema 2: Instalación de SIVIREP
- Tema 3: Verificación funcionamiento
...
[covid_latam.xlsx](https://raw.githubusercontent.com/isp-javeriana/JornadasEpiBogota2025/main/data/covid_latam.xlsx):

Dentro del directorio en el que está trabajando actualmente, cree una carpeta llamada *data*. Guarde la tabla de datos descargado en la carpeta *data* que acaba de crear.

Para importar tablas de datos desde R, se puede usar la función `read_excel`, del paquete `readxl`:

```{r, eval=FALSE}
library(readxl)
covid_latam <- read_excel("data/covid_latam.xlsx")
...
```{r, error=TRUE, eval = FALSE}
covid_latam %>% summarise(media = mean(edad), numero = n())
```
...
```{r, error=TRUE, eval = FALSE}
covid_latam %>% select(edad) #empleando el nombre de la columna
covid_latam %>% select(c(1,2)) #o su ubicación en los datos
```
...
```{r, error=TRUE, eval = FALSE}
covid_latam %>% glimpse() #Observe como están expresadas las variables también puede usar la función `table()`
covid_latam %>% filter(sexo == "femenino", edad <= 28) #Ahora sabe como filtrar el sexo
```
...
```{r, error=TRUE, eval = FALSE}
covid_latam %>% arrange(edad)
covid_latam %>% arrange(edad,sexo)
```
...
```{r, error=TRUE, eval = FALSE}
unique(covid_latam$nombre_del_pais)
covid_latam <- covid_latam %>% mutate(nombre_del_pais = toupper(nombre_del_pais))
```
...
```{r, error=TRUE, eval = FALSE}
covid_latam <- covid_latam %>% rename(pais = nombre_del_pais)
```
...
```{r, error=TRUE, eval = FALSE}
slice(filter(group_by(covid_latam, sexo),edad >= 13, edad <=14),1:5)
```
...
Esta vez se cargará un tipo diferente de datos, estos los puede encontrar en el enlace <https://github.com/isp-javeriana/JornadasEpiBogota2025/blob/main/data/datos_covid.RDS?raw=true>.
...
```{r, error=TRUE, eval = FALSE}
url <- "https://github.com/isp-javeriana/JornadasEpiBogota2025/blob/main/data/datos_covid.RDS?raw=true"
